...
function classical_graph.draw(graph, wibox, cr, width, height)
    local graph_surface=cairo.ImageSurface.create("argb32",data[graph].width, data[graph].height)
    local graph_context = cairo.Context.create(graph_surface)
	...
	helpers.draw_rounded_corners_rectangle( graph_context, 0, 0, data[graph].width, data[graph].height, data[graph].background_color, rounded_size )
    end
...
--function classical_graph:set_height(height)
--    if height >= 5 then
...
--function classical_graph:set_width(width)
--    if width >= 5 then
...
for _, prop in ipairs(properties) do
    if not classical_graph["set_" .. prop] then
	classical_graph["set_" .. prop] = function(graph, value)
...
function classical_graph.new(args)
    local args = args or {}
    ...
    -- Set methods
    for _, prop in ipairs(properties) do
	graph["set_" .. prop] = classical_graph["set_" .. prop]
    end

    graph.draw = classical_graph.draw
    --graph.add_value = add_value

    return graph
end

function classical_graph.mt:__call(...) 
    return classical_graph.new(...)
end


return setmetatable(classical_graph, classical_graph.mt)
