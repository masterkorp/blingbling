...
function helpers.draw_background_tiles(cairo_context, height, v_margin , width, h_margin)
    ...
    local x=width-(tiles_width + v_rest)
    local y=height -(v_margin +tiles_height + h_rest) 
...
function helpers.draw_vertical_bar(cairo_context,h_margin,v_margin, width,height, represent)
    local x=h_margin
    local bar_width=width - 2*h_margin
    local bar_height=height - 2*v_margin
    local y=v_margin 
    local r, g, b, a
    ...
    local gradient=cairo.Pattern.create_linear(h_margin, height/2, width-h_margin, height/2)
...
--@param border color a string "rrgggbb" or "rrggbbaa" for the color of the border
function helpers.draw_rounded_corners_rectangle(cairo_context,x,y,width, height, color, rounded_size, border)
    ...
    if border ~= nil then
	cairo_context:set_line_width(1)
	local r, g, b, a = helpers.hexadecimal_to_rgba_percent(border)
	cairo_context:set_source_rgba(r, g, b, a)
...
function helpers.draw_rounded_corners_horizontal_graph(cairo_context,x,y,width, height, background_color, graph_color, rounded_size, value_to_represent, graph_line_color)
    ...
    --draw the background
    local r,g,b,a=helpers.hexadecimal_to_rgba_percent(background_color)
    cairo_context:set_source_rgba(r,g,b,a)
    --top left corner
    cairo_context:arc(x + radius*rounded_size,y + radius*rounded_size, radius*rounded_size,PI, PI * 1.5)
    --top right corner
    ...
    local value = value_to_represent
    local limit_2 = (width -(radius * rounded_size)) / width
    local limit_1 = radius* rounded_size /width
...
function helpers.generate_rounded_rectangle_with_text_in_image(text, padding, background_color, text_color, font_size, rounded_size)
    ...
    --draw the background
    helpers.draw_rounded_corners_rectangle(cr,0,0,data.width, data.height, background_color, rounded_size)
